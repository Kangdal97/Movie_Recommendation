...
try:
    for i in range(1, 44):  # 페이지
        ...
        for j in range(1, 21):  # 한페이지 최대 타이틀
            print(j + ((i - 1) * 20), '번째 영화({}페이지{}번째) 크롤링 중 / ({}/850)'.format(i, j, (j + ((i - 1) * 20))))
            try:
                driver.get(url)
        ...
        df_review_20.to_csv('./crawling/reviews_{}_{}.csv'.format(2019, i), index=False)
...
# df_review.to_csv('./crawling/reviews_{}.csv'.format(2020) ,index=False)
