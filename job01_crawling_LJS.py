    ...
    for i in range(1, 38):  # 총 37페이지
        ...
        for j in range(1, 21):
            ...
            try:
                ...
                review_range = int(driver.find_element_by_xpath(review_number_xpath).text.replace(',',
                                                                                                  ''))  # 총 리뷰건수 예를들면 1,234->1234로 바꿔줌
                ...
                for k in range(1, review_range):
                    ...
                    time.sleep(0.3)
                    for l in range(1, 11):
                        review_title_xpath = '//*[@id="reviewTab"]/div/div/ul/li[{}]/a/strong'.format(
                            l)  # 첫번째 리뷰 제목 크롤링
                        try:
                            driver.find_element_by_xpath(review_title_xpath).click()  # 첫번째 리뷰 제목 클릭
                            time.sleep(0.3)
                            review = driver.find_element_by_xpath(
                                '//*[@id="content"]/div[1]/div[4]/div[1]/div[4]').text  # 리뷰 내용 전부 크롤링
                            # print('================================= =================================')
                            # print(title)
                            # print(review)
                            titles.append(title)
        ...
        df_review_20.to_csv('./crawling/reviews_{}_{}.csv'.format(2020, i), index=False)
except:
    print('totally error')
finally:  # 에러나든 안나든 모조건 실행
    driver.close()
# df_review = pd.DataFrame({'title':titles, 'reviews':'reviews'})
# df_review.to_csv('./crawling/reviews_{}.csv').format(2020)
